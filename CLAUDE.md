...
- **Nie pobieraj nowych lekcji z 10x-cli** (`npx @przeprogramowani/10x-cli@latest get ...`) bez wyraźnego polecenia użytkownika. Dotyczy to każdej lekcji, modułu lub materiału kursu.
...
- **Supabase migrations**: `supabase/migrations/` using naming format `YYYYMMDDHHmmss_short_description.sql`. Always enable RLS on new tables with granular per-operation, per-role policies. Before running `supabase db push` on a fresh checkout, run `supabase link --project-ref hfiasswaduellpweeloc` first (`.supabase/` is gitignored).
- **React**: no Next.js directives ("use client" etc.). Extract hooks to `src/components/hooks/`.
- **Services/helpers** go in `src/lib/` (or `src/lib/services/` for extracted business logic).
- **Shared types**: `src/types/` directory — auto-generated DB/schema types in `src/types/supabase.ts` (regenerate with `supabase gen types typescript --project-id <ref>`); handcrafted entities/DTOs in `src/types/domain.ts`.
